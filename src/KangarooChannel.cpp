  ...
  KangarooGetFlags flags)
{
  ...
  if (getInitialSequenceCodeIfNecessary(timeout, initialStatus)) {
  ...
  size_t valueCount)
{
  ...
  for (size_t i = 0; i < valueCount; i++) {
    contents.writeBitPackedNumber(values[i]);
  }
  ...
  const KangarooTimeout & timeout)
{
  ...
  while (1) {
    ...
    if (receiver.command() != KANGAROO_RC_STATUS) {
      continue;
    }
  ...
  KangarooMoveFlags moveFlags)
{
  ...
  KangarooCommandWriter writer;
  writer.write((uint8_t)name());
  writer.write((uint8_t)moveFlags);
  ...
  KangarooGetType getType)
{
  ...
  if (streaming()) {
    ...
    if (!getInitialSequenceCodeIfNecessary(timeout, _monitoredGetResult)) {
      ...
      // Make sure the packet was received correctly -- proper sequence code and
      // the like. If not, we'll resend.
...
// Internally generated errors (like timeouts) have negative numbers, so we pass
